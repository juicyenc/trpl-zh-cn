...
当创建新的 `Post` 时，我们将其 `state` 字段设置为一个存放了 `Box` 的 `Some` 值。这个 `Box` 指向一个新的 `Draft` 结构体实例。这确保了无论何时新建一个 `Post` 实例，它都会从草稿开始。因为 `Post` 的 `state` 字段是私有的，因此也就无法创建任何其他状态的 `Post` 了！在 `Post::new` 函数中 `content` 将被设置为新的、空的 `String`。
